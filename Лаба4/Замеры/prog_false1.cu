...
__global__ void change_rows(el* arr, int n, int cur) {
  ...
  double coef;
  for (int y = cur + idy + 1; y < n; y += offsety){
    coef = -arr[n * cur + y].val / arr[n * cur + cur].val;
    for (int x = cur + idx + 1; x < n + 1; x += offsetx) {
       arr[n * x + y].val = arr[n * x + y].val + arr[n * x + cur].val * coef;
    }
  }
}
